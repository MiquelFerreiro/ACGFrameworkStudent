...
uniform vec4 u_color;               // Used as emitted color Le
uniform vec4 u_background_light;     // Background color B
//uniform float u_step_length;         // Step length (controlled by GUI)
uniform float u_absorption_coeff;    // Absorption coefficient μa

out vec4 FragColor;

// Noise function for heterogeneous volume
float hash1(float n) {
...
void main() {
    ...
    for (float t = tNear; t < tFar; t += u_step_length) {
        float density = noise(position * 5.0);  // Sample 3D noise as density
        float absorption = density * u_absorption_coeff;
